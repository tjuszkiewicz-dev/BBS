...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 bg-slate-900 text-white shadow-xl border-r border-slate-800 transition-all duration-300 ease-in-out flex flex-col flex-shrink-0 overflow-hidden
        ${isOpen ? 'translate-x-0 w-72' : '-translate-x-full w-72'} 
        md:translate-x-0 md:shadow-none
        ${isDesktopOpen ? 'md:w-72' : 'md:w-0'}
      `}>
        {/* Brand Header */}
        <div className="h-32 flex items-center justify-between px-6 border-b border-slate-800 bg-slate-900 whitespace-nowrap min-w-[18rem]">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="BBS Logo" className="w-28 h-28 object-contain" />
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-white leading-tight">BBS</h1>
            </div>
          </div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto no-scrollbar whitespace-nowrap min-w-[18rem]">
          <p className="px-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-3">Menu Systemowe</p>
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => {
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 group ${
                currentView === item.id
                  ? 'bg-bbs-500 text-white shadow-md shadow-bbs-500/30'
                  : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
            >
              <span className={currentView === item.id ? 'text-white' : 'text-slate-500 group-hover:text-bbs-400'}>
                {item.icon}
              </span>
              <span>{item.label}</span>
              {currentView === item.id && <ChevronRight size={16} className="ml-auto opacity-70" />}
            </button>
          ))}
        </nav>

        {/* User Profile / Logout (Bottom Sticky) */}
        <div className="p-4 bg-slate-950 border-t border-slate-800 whitespace-nowrap min-w-[18rem]">
          <button 
            onClick={onSwitchUser}
            className={`w-full flex items-center gap-3 p-3 rounded-xl transition-colors group text-left relative overflow-hidden ${
                isLogout ? 'bg-red-950/50 hover:bg-red-900/50 text-red-400 border border-red-900/50' : 'bg-slate-800 hover:bg-slate-700 border border-slate-700'
            }`}
          >
             <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-sm flex-shrink-0 ${
                 isLogout ? 'bg-red-900/50 text-red-400' : 'bg-gradient-to-br from-bbs-400 to-bbs-700 text-white'
             }`}>
                {isLogout ? <LogOut size={18}/> : currentUser.name.charAt(0)}
             </div>
             
             <div className="flex-1 min-w-0">
                <p className={`text-sm font-bold truncate ${isLogout ? 'text-red-400' : 'text-white'}`}>
                    {isLogout ? 'Wyloguj się' : currentUser.name}
                </p>
                <div className="flex items-center gap-1.5 text-xs font-medium">
                  <span className={`w-1.5 h-1.5 rounded-full ${isLogout ? 'bg-red-500' : 'bg-bbs-400'} animate-pulse`} />
                  <span className={`truncate ${isLogout ? 'text-red-500' : 'text-slate-400'}`}>{getRoleLabel(currentUser.role)}</span>
                </div>
             </div>
          </button>
